...
router.put('/favorite-vocabularies', async (req, res, next) => {
    UserModel.findByIdAndUpdate(req.user._id,
        {$addToSet : {vocabularies: req.body.vocabularyId}}, {new: true}, async (err, u) => {
            if(err) {
                console.error(err);
                return next(err);
            }
...
router.get('/favorite-vocabularies', async (req, res, next) => {
    UserModel.findById(req.user._id, 'username firstName lastName vocabularies').populate('vocabularies').exec(async (err, u) => {
        if(err) {
            console.error(err);
            return next(err);
        }
...
router.delete('/favorite-vocabularies/:vId', async (req, res, next) => {
    UserModel.findByIdAndUpdate(req.user._id,
        {$pull : {vocabularies: req.params.vId}}, {new: true}, async (err, u) => {
            if(err) {
                console.error(err);
                return next(err);
            }
